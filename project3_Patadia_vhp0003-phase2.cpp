...
bool compareTimestamps(const ExperimentData &a, const ExperimentData &b) {
...
double calculateMedian(const vector<ExperimentData> &data) {
...
int main() {
    ...
    // Value-based sorting and statistics
    ...
    // Chronological sorting and statistics
    vector<ExperimentData> chronoData = allData;
    sort(chronoData.begin(), chronoData.end(), compareTimestamps);
    
    cout << "The chronologically sorted list of " << chronoData.size() << " values is:\n";
    for (size_t i = 0; i < chronoData.size(); i++) {
        cout << fixed << setprecision(2) << chronoData[i].value;
        if (i < chronoData.size() - 1) cout << " ";
    }
    cout << "\n\n";

    cout << "The mean is " << calculateMean(chronoData) << "\n";
    cout << "The median is " << calculateMedian(chronoData) << "\n";
    cout << "The mode is " << calculateMode(chronoData) << "\n";
    ...
    if (outFile.is_open()) {
        ...
        // Chronologically sorted statistics
        outFile << "The chronologically sorted list of " << chronoData.size() << " values is:\n";
        for (size_t i = 0; i < chronoData.size(); i++) {
            outFile << chronoData[i].value;
            if (i < chronoData.size() - 1) outFile << " ";
        }
        outFile << "\n\n";
        outFile << "The mean is " << calculateMean(chronoData) << "\n";
        outFile << "The median is " << calculateMedian(chronoData) << "\n";
        outFile << "The mode is " << calculateMode(chronoData) << "\n";
